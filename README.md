...
- **Notification outbox (dev provider)**: foundation untuk appointment/check-in confirmation. Mask + SHA-256 dedup, dev/local provider only, no real vendor, + notification-worker (manual run) dengan template renderer (8 placeholder variabel, allow-list ketat). Lihat [docs/NOTIFICATIONS_REPORT.md](docs/NOTIFICATIONS_REPORT.md).
